...
ParameterKey=SiteName,ParameterValue="Rancherrke1" \
...
while [[ $(aws cloudformation describe-stacks --stack-name Rancherrke1 --query "Stacks[].StackStatus" --output text) != "CREATE_COMPLETE" ]];
do
     RESULT=$(aws cloudformation describe-stacks --stack-name Rancherrke1 --query "Stacks[].StackStatus" --output text)
     echo "$RESULT waiting for cloudformation stack to complete."
     sleep 60
done
aws cloudformation describe-stacks --stack-name Rancherrke1 --query "Stacks[0].Outputs[]" --output table
echo "All Done"
